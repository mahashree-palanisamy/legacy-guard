...
const AssetsPage = () => {
  ...
  const openEdit = (a: Asset) => {
    if (!isOwner) { toast.error('You have view-only access'); return; }
    setForm({ name: a.name, type: a.type, username: a.username, password: a.password, notes: a.notes }); setEditingId(a.id); setModalOpen(true);
  };

  const handleSave = () => {
    if (!isOwner) { toast.error('You have view-only access'); return; }
    if (!form.name.trim()) { toast.error('Asset name is required'); return; }
    if (editingId) { updateAsset(editingId, form); toast.success('Asset updated'); }
    else { addAsset(form); toast.success('Asset added'); }
    setModalOpen(false);
  };

  const handleDelete = (id: string) => {
    if (!isOwner) { toast.error('You have view-only access'); return; }
    deleteAsset(id); toast.success('Asset deleted');
  };
    ...
    <div className="page-container">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {isOwner ? 'Manage your digital assets and credentials' : 'Viewing inherited digital assets'}
          </p>
        </div>
        {isOwner && (
      ...
      {!isOwner && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/10">
          ...
          <p className="text-xs text-muted-foreground">You have view-only access to these assets.</p>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
        {filtered.map((a) => (
          <AssetCard key={a.id} asset={a} showActions={isOwner} onEdit={() => openEdit(a)} onDelete={() => handleDelete(a.id)} />
        ))}
      </div>

      {filtered.length === 0 && (
        <div className="text-center py-16"><p className="text-muted-foreground">No assets found.</p></div>
      )}
