...
const HeirsPage = () => {
  ...
  const openEdit = (h: typeof heirs[0]) => { setForm({ name: h.name, email: h.email, relationship: h.relationship }); setEditingId(h.id); setModalOpen(true); };
    ...
    <div className="page-container">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage your heirs and asset assignments</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
        {heirs.map((h) => (
          <div key={h.id}>
            <HeirCard heir={h} onEdit={() => openEdit(h)} onDelete={() => { deleteHeir(h.id); toast.success('Heir deleted'); }} />
